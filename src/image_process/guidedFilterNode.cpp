...
namespace pl2i2_slam
{
    ...
    private:
        ...
        std::string outFolder = "/home/jgl/Documents/Projects/PL2I2-SLAM/src/PL2I2-SLAM/imgs/guided_filter/";
    ...
    public:
        ...
        void run()
        {
            while (true)
            {
                if (!msgBuf.empty())
                {
                    ...
                    ROS_INFO("[Guided Filter] img_0 total: %lf ms", tictoc.toc());
                    tictoc.tic();
                    cv::Mat outImg_1 = dehaze(cvPtr->image, 0.1, 15, 0.4, 0.75, 0.1, 1e-3, true);
                    ROS_INFO("[Guided Filter] img_1 total: %lf ms", tictoc.toc());
        ...
        */
        cv::Mat dehaze(cv::Mat img, float tmin = 0.1, int w = 15, float alpha = 0.4, float omega = 0.75, float p = 0.1, float eps = 1e-3, bool reduce = false)
        {
            ...
            ROS_INFO("[Guided Filter] get bright/dark channels: %lf ms", _clock.toc());
            output_intermediate_image(Idark, "darkch");
            output_intermediate_image(Ibright, "brightch");

            _clock.tic();
            cv::Mat A = get_atmosphere(img, Ibright);
            std::cout << "A = " << cv::format(A, cv::Formatter::FMT_C) << std::endl;
            ROS_INFO("[Guided Filter] get global illumination atmosphere: %lf ms", _clock.toc());

            _clock.tic();
            cv::Mat init_t = get_init_transmission(A, Ibright);
            output_intermediate_image(init_t, "init_t");
            ROS_INFO("[Guided Filter] get init transmission: %lf ms", _clock.toc());
            ...
            output_intermediate_image(corrected_t, "corrected");
            ROS_INFO("[Guided Filter] get corrected transmission: %lf ms", _clock.toc());
            ...
            output_intermediate_image(J_refined, "guided_filter_only");
            ROS_INFO("[Guided Filter] guided filter only: %lf ms", _clock.toc());
            ...
            output_intermediate_image(f_enhanced, "guided_filter_enhanced");
            ROS_INFO("[Guided Filter] guided filter with enhancement: %lf ms", _clock.toc());
            ...
            else
            {
                for (int i = padding; i < padding + width; i++)
                {
                    for (int j = padding; j < padding + height; j++)
                    {
                        ...
                        padded.at<cv::Vec3f>(j, i).val[1] = (float)I.at<cv::Vec3f>(j - padding, i - padding).val[0] / 255;
                        padded.at<cv::Vec3f>(j, i).val[2] = (float)I.at<cv::Vec3f>(j - padding, i - padding).val[0] / 255;
                    }
                }
            }
...
int32_t main(int32_t argc, char **argv)
{
    ...
    ros::NodeHandle nh;
    pl2i2_slam::GuidedFilter guidedFilter("/thermal_image_raw", 1000);
