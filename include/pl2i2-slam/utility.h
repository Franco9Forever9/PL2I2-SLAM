...
namespace pl2i2_slam
{
    ...
    /*
    * 时钟类型：
    *   system_clock： 系统范围的挂钟，可以调节，因此返回值不一定单调；
    *   steady_clock： 稳定单调的始终，不受系统时间的影响，适用于测量时间间隔。
    * 
    * 时间点：
    *   now(): 每个时钟都具有的静态方法，用于返回当前时间点；
    *   is_steady： 每个时钟都具有的常量，用于表示时钟是否单调；
    *   period： 每个时钟都具有的成员类型，用于定义一个时钟周期的时长，分辨率是纳秒（以秒为单位）
    * 
    */
    class TicToc
    {
